...
vector<errors> calcSequenceErrors (vector<Matrix> &poses_gt,vector<Matrix> &poses_result) {
  ...
  for (int32_t first_frame=0; first_frame<poses_gt.size(); first_frame+=step_size) {
    ...
    for (int32_t i=0; i<num_lengths; i++) {
      ...
      // write to file
      err.push_back(errors(first_frame, (r_err/len) * (180.0 / 3.1415926), t_err/len * 100.0 ,len,speed));
    }
  }
...
void saveErrorPlots(vector<errors> &seq_err,string plot_error_dir,char* prefix) {
  ...
  for (int32_t i=0; i<num_lengths; i++) {
    ...
    if (num>2.5) {
      fprintf(fp_tl,"%f %f %f\n",lengths[i],t_err/num, num);
      fprintf(fp_rl,"%f %f %f\n",lengths[i],r_err/num, num);
    }
  }
...
bool eval (string result_sha,Mail* mail) {
  ...
  string base_dir       = "/Users/robweston/code/devkit";
  string gt_dir         = base_dir + "/data/" + result_sha + "/gt";
  string result_dir     = base_dir + "/data/" + result_sha + "/pred";
  //string result_dir     = "/Users/robweston/code/devkit/results/" + result_sha;
  
  string error_dir      = base_dir + "/results/" + result_sha + "/errors";
  //string plot_path_dir  = base_dir + "/results/" + result_sha + "/avg";
  string plot_error_dir = base_dir + "/results/" + result_sha + "/avg";
  ...
  for (int32_t i=11; i<12; i++) {
    ...
    cout << poses_gt[100] << endl;
    cout << poses_result[100] << endl;
  ...
  //  plotErrorPlots(plot_error_dir,prefix);
    saveStats(total_err, plot_error_dir);
  }
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  if (argc < 2) {
    cout << "Usage: ./eval_odometry pose_seq [pos_seq]*" << endl;
    return 1;
  }
  cout << argc << endl;
  cout << argv[0] << endl;
